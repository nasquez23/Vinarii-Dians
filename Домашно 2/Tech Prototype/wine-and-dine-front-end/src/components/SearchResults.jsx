...
function Result(props){
    ...
    return(<div id={'result'} onClick={highlight} style={{backgroundColor : bgColor}}>
        <h4>{name}</h4>
        <div>{address}</div>
        <div>{phone}<a href={String(site)}> {site}</a></div>
        <div>{openHours} - {closeHours}</div>
    </div>)
...
export default function SearchResults(){
    ...
    return(<div id={'resultsContainer'}>
        {wineries.length===0?"No results":wineries.map(marker=><Result winery={marker} key={marker.name} />)}
    </div>)
}